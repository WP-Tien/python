...
def items_article_sidebar_recent(request):
    ...
    items_article_sidebar_recent = Article.objects.filter(status=APP_VALUE_STATUS_ACTIVE, publish_date__lte = timezone.now()).exclude(slug=skip_slug).order_by('-publish_date')[:SETTING_ARTICLE_TOTAL_ITEMS_RELATED]
...
def items_article_footer_random(request):
    ...
    items_article_footer_random = Article.objects.filter(status=APP_VALUE_STATUS_ACTIVE, publish_date__lte = timezone.now()).exclude(slug=skip_slug).order_by('?')[:3]
